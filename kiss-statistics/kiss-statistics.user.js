...
function getCatArray() {
	...
	if(SITE_TYPE === SITE_MANGA){return new Array();}
	...
	if(SITE_TYPE === SITE_CARTOON){selector = 'tr.trCartoon';}
	if(SITE_TYPE === SITE_DRAMA){selector = 'tr.trDrama';}
	jQuery(selector).each(function(index){
		// get Category val
		var tmpCat = jQuery(this).attr('catname');
		// imitate lstCats behaviour
		if(tmpCat === '') {tmpCat = STR_NO_CATEGORY;}
...
function getSiteType() {
	...
	if (url.indexOf('kissmanga') != -1) {return SITE_MANGA;}
	if (url.indexOf('kisscartoon') != -1) {return SITE_CARTOON;}
	if (url.indexOf('kissasian') != -1) {return SITE_DRAMA;}
	return SITE_ANIME;
}

/* Adds an 's' if appropriate (basically add it if the Kiss site does so too) */
function getPluralS() {
	if(SITE_TYPE === SITE_ANIME) {return '';}
	if(SITE_TYPE === SITE_MANGA) {return 's';}
	if(SITE_TYPE === SITE_CARTOON) {return 's';}
	if(SITE_TYPE === SITE_DRAMA) {return '';}
	return '';
}
...
function getTotalSize(categoryName) {
	...
	if (SITE_TYPE === SITE_MANGA) {return JQ('.aUnRead').size();}
	...
	if (categoryName === STR_NO_CATEGORY) {
		if (SITE_TYPE === SITE_CARTOON) {return JQ("tr.trCartoon[catName='']").size();}
		if (SITE_TYPE === SITE_DRAMA) {return JQ("tr.trDrama[catName='']").size();}
		return JQ("tr.trAnime[catName='']").size();
	}

	// non special categories
	if (SITE_TYPE === SITE_CARTOON) {return JQ("tr.trCartoon[catName='" + categoryName + "']").size();}
	if (SITE_TYPE === SITE_DRAMA) {return JQ("tr.trDrama[catName='" + categoryName + "']").size();}
...
function getNonCompletedSize(categoryName) {
	// handle KissManga separately as it does not have categories/folders and no link sharing option
	if (SITE_TYPE === SITE_MANGA) {return JQ(SELECTOR_VISIBLE).size();}
	...
	// handle the special 'Uncategorized' category
	if (categoryName === STR_NO_CATEGORY) {
		if (SITE_TYPE === SITE_CARTOON) {return JQ("tr.trCartoon[catName=''] > td > " + SELECTOR_VISIBLE ).size();}
		if (SITE_TYPE === SITE_DRAMA) {return JQ("tr.trDrama[catName=''] > td > " + SELECTOR_VISIBLE ).size();}
		return JQ("tr.trAnime[catName=''] > td > " + SELECTOR_VISIBLE ).size();
	}
	// non special categories
	if (SITE_TYPE === SITE_CARTOON) {return JQ("tr.trCartoon[catName='"+ categoryName + "'] > td > " + SELECTOR_VISIBLE ).size();}
	if (SITE_TYPE === SITE_DRAMA) {return JQ("tr.trDrama[catName='"+ categoryName + "'] > td > " + SELECTOR_VISIBLE ).size();}
...
function createHtmlSkeleton() {
	...
	if(SITE_TYPE === SITE_MANGA) {catSelect = '.listing';}
